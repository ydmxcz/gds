...
func (l *List[T]) iter(node *Element[T]) iterator.Iter[T] {
	return func() (val T, ok bool) {
		if ok = node != nil; ok {
			val = node.Value
			node = node.Next()
...
func (l *List[T]) iterStep(step int, node *Element[T]) iterator.Iter[T] {
	return func() (val T, ok bool) {
...
func (l *List[T]) SplitableIter() func(parallelism int) iterator.Iter[iterator.Iter[T]] {
	return func(parallelism int) iterator.Iter[iterator.Iter[T]] {
		...
		return func() (pull iterator.Iter[T], ok bool) {
			...
			if i+step >= l.len {
				return l.iter(n), true
			}
			return l.iterStep(step, n), true
		}
	}
}
